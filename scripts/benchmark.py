...
def worker(n, relax_iterations=0):
    ...
    result = {"atoms": n, "first_evaluation_seconds": first,
    ...
    if relax_iterations:
        ...
        result.update(relaxation_backend="native", relaxation_seconds=time.perf_counter()-start,
                      relaxation_iterations=relaxed.iterations, relaxation_converged=relaxed.converged,
...
def main():
    ...
    parser.add_argument("--relax-iterations", type=int, default=0,
    ...
    report = {"date": datetime.now(timezone.utc).isoformat(), "python": sys.version,
              ...
              "environment": os.environ.get("CONDA_DEFAULT_ENV"), "executable": sys.executable,
              "full_derivative": False, "force_convention": "fixed_charge",
              ...
              "results": []}
    for n in (20, 100, 200):
        child = subprocess.run([sys.executable, __file__, "--worker", str(n),
