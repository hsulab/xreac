...
def image_qeq(ff, symbols, x, cell):
    ...
    bond_candidates = mask & (r <= min(5., cutoff))
    ...
    for i, s in enumerate(symbols):
        for j, t in enumerate(symbols):
            ...
            for atom_radius, pair_radius, power1, power2, factor in (
                ("r_s", "r_s", "p_bo1", "p_bo2", 1+.01*ff.general[29]),
                ("r_pi", "r_p", "p_bo3", "p_bo4", 1.),
                ("r_pi_pi", "r_pp", "p_bo5", "p_bo6", 1.),
            ):
    ...
    active_bonds = bond_candidates & (raw >= .01*ff.general[29])
    return {
        "charges": charges.tolist(), "coupling": coupling.tolist(),
...
def differences(primitive, expanded, copies, n):
    ...
    result = {"energy_difference_per_primitive_cell": primitive.energy-expanded.energy/copies,
    ...
    result["consistent"] = (abs(result["energy_difference_per_primitive_cell"])/n <= 1e-5
...
def main():
    ...
    cases = [(f"water_{side:g}A", water, symbols, x, np.diag([side]*3))
             for side in (12., 9., 6., 4., 3.12)]
    ...
    for name, ff, symbols, x, cell in cases:
        ...
        minimum = max(ff.general[12], 2*min(5., ff.general[12]))
        ...
        (work/"structure.json").write_text(json.dumps(dict(symbols=symbols, positions=x.tolist(),
             cell=cell.tolist(), pbc=[True]*3, repeats=repeats.tolist()), indent=2)+"\n")
        row = {"primitive_atoms": n, "cell": cell.tolist(), "repeats": repeats.tolist(),
               "expanded_atoms": len(sx), "force_field_sha256": ff.checksum}
        try:
        ...
        row["explicit_qeq_supercell_charge_difference"] = float(np.max(abs(
            np.tile(independent["charges"], copies)-expanded.charges)))
        row["reference_version"] = expanded.version
        try:
            small = evaluate_lammps(ff, symbols, x, cell=cell, allow_small_cell=True,
                                     directory=work/"lammps_primitive")
            ...
            row["explicit_qeq_primitive_charge_difference"] = float(np.max(abs(
                np.asarray(independent["charges"])-small.charges)))
        except RuntimeError as exc:
            row["primitive_lammps_status"] = str(exc)
        report["cases"][name] = row
        print(name+": "+json.dumps({"supercell_verified": row["supercell_python_lammps"]["passed"],
              "primitive_vs_supercell": row.get("primitive_vs_supercell", row["primitive_lammps_status"])}), flush=True)
        (args.output/"summary.json").write_text(json.dumps(report, indent=2)+"\n")
    report["supported_supercells_passed"] = all(row["supercell_python_lammps"]["passed"]
                                               for row in report["cases"].values())
    report["explicit_qeq_supercells_passed"] = all(row["explicit_qeq_supercell_charge_difference"] <= 1e-6
                                                 for row in report["cases"].values())
    (args.output/"summary.json").write_text(json.dumps(report, indent=2)+"\n")
