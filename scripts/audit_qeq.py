...
def main():
    ...
    calc = Calculator(ff)
    report = {"force_field_sha256": ff.checksum,
              "energy_units": "kcal/mol", "force_units": "kcal/mol/Angstrom",
              "charge_units": "e", "qeq_residual_units": "eV",
              "qeq_tolerance": 1e-12, "cases": {}}
    for name in ("monomer", "dimer"):
        ...
        row = {"reference_version": ref.version, "atom_index_one_based": int(index[0])+1,
        ...
        for step in (1e-3, 1e-4, 1e-5):
            ...
            for sign, label in ((1, "plus"), (-1, "minus")):
                ...
                displaced.append(evaluate_lammps(ff, symbols, positions,
                    directory=directory / f"h-{step:g}-{label}"))
            ...
            row["finite_differences"].append({
                "step_A": step, "energy_plus": plus.energy, "energy_minus": minus.energy,
        ...
        print(f"{name}: LAMMPS force={ref.forces[index]:.10f}; LAMMPS energy derivative={final['force_from_lammps_energies']:.10f}; QEq residual={residual:.3e}", flush=True)
