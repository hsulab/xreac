...
def main():
    ...
    for name, ff, (symbols, x), tolerance in cases:
        ...
        row.update({"iterations": relaxed.iterations, "converged": relaxed.converged,
                    "message": relaxed.message, "force_tolerance": tolerance,
                    "max_iterations": 500, "initial_energy": initial.energy,
                    ...
                    "reference_version": ref.version, "force_field_sha256": ff.checksum})
        row["passed"] = bool(row["passed"] and row["converged"] and not relaxed.evaluation.full_derivative
                             and row["max_force"] <= tolerance and row["reference_max_force"] <= tolerance+1e-7)
        report["cases"][name] = row
        print(f"{name}: {'PASS' if row['passed'] else 'FAIL'}; {relaxed.iterations} iterations; max force {row['max_force']:.3e}; LAMMPS {row['reference_max_force']:.3e}", flush=True)
