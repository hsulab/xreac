...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "validation" / "runs" / ("neighbors-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")),
    )
    ...
    for name, (filename, symbols, x, cell, pbc) in neighbor_cases().items():
        ...
        if args.verify:
            ref = evaluate_lammps(
                ff, symbols, x, cell=cell, pbc=pbc, directory=work / "lammps", executable=args.executable
            )
