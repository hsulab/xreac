...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "validation" / "runs" / datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ"))
    ...
    calc = Calculator(ff)
    cases = dict(CASES)
    for r in (1.5, 2.5, 3., 3.5, 4., 4.9999, 5.0001, 9.999, 10., 10.001):
        cases[f"stretch_{r}"] = (["Zn", "O"], [[0, 0, 0], [r, 0, 0]])
    relaxed = calc.relax(["Zn", "O"], [[0, 0, 0], [2.3, .1, .2]], force_tolerance=1e-5)
    ...
    report = {"force_field_sha256": ff.checksum, "reference_version": None,
              "energy_tolerance_per_atom": 1e-5, "charge_tolerance": 1e-6,
