...
THB_CUT = 0.001
THB_PRODUCT_CUT = 0.00001
BOND_CUT = 5.0
HBOND_CUT = 7.5
HBOND_THRESHOLD = 0.01
BOND_GRAPH_CUT = 0.3
COMPONENTS = (
    "bond",
    "atom",
...
class EnergyModel:
    ...
    Neighbor construction is the caller's responsibility. All atom properties,
    ...
    def electrostatics(self, r, fixed_charges=None):
        ...
        taper = np.where(r < self.g[12], 1 + u**4 * (-35 + u * (84 + u * (-70 + 20 * u))), 0.0)
    ...
    def bond_orders(self, r):
        ...
        sigma = np.where(
            sigma_ok, (1 + cutoff) * np.exp(p["p_bo1"] * (r / np.where(sigma_ok, p["r_s"], 1)) ** p["p_bo2"]), 0
        )
        ...
        d, db = total - a["valency"], total - a["valency_val"]
        f2 = np.exp(-g[0] * d[i]) + np.exp(-g[0] * d[j])
        f3 = -np.log(0.5 * (np.exp(-g[1] * d[i]) + np.exp(-g[1] * d[j]))) / g[1]
        vi, vj = a["valency"][i], a["valency"][j]
        f1 = np.where(p["ovc"] >= 0.001, 0.5 * ((vi + f2) / (vi + f2 + f3) + (vj + f2) / (vj + f2 + f3)), 1)
    ...
    def properties(self, x, charges, bond_state):
        ...
        return dict(
            ...
            total_bond_orders=total,
            lone_pairs=nlp,
            bond_counts=self.edges.atom_sum((getval(bo) > BOND_GRAPH_CUT).astype(float)).astype(int),
    ...
    def components(self, x, fixed_charges=None, *, return_bond_state=False):
        ...
        dlp = 0.5 * (a["valency_e"] - a["valency"]) - nlp
        dlpt = np.where(a["mass"] > 21, 0, dlp)
        e = {k: 0.0 for k in COMPONENTS}
        e["bond"] = -0.5 * np.sum(
            p["De_s"] * sigma * np.exp(p["p_be1"] * (1 - positive_power(sigma, p["p_be2"])))
            + p["De_p"] * pi
            + p["De_pp"] * pp
        )
        ...
        under = (
            -a["p_ovun5"]
            * (1 - np.exp(g[6] * dc))
            / (1 + np.exp(-a["p_ovun2"] * dc))
            / (1 + g[8] * np.exp(g[9] * ov2))
        )
        e["atom"] = np.sum(over + under)
        f13 = (r ** g[28] + (1 / p["gamma_w"]) ** g[28]) ** (1 / g[28]) if self.vdw_type in (1, 3) else r
        ev = np.exp(0.5 * p["alpha"] * (1 - f13 / p["r_vdW"]))
        e["vdw"] = 0.5 * np.sum(taper * p["D"] * (ev**2 - 2 * ev))
        if self.vdw_type in (2, 3):
            e["vdw"] += 0.5 * np.sum(taper * p["ecore"] * np.exp(p["acore"] * (1 - r / p["rcore"])))
        e["coulomb"] = 0.5 * C_ELE * np.sum(q[i] * q[j] * shield)
        e["qeq"] = SELF_CONVERSION * np.sum(a["chi"] * q + 0.5 * a["eta"] * q * q)
    ...
    def special_bond_corrections(self, e, bo, total, d):
        ...
        if g[5] > 0.001 and onp.any(carbon):
            vov3 = bo - d[i] - 0.040 * d[i] ** 4
            mask = carbon[i] & carbon[j] & (getval(vov3) > 3)
            e["lone_pair"] += np.sum(np.where(mask, g[5] * (vov3 - 3) ** 2, 0))
        masses = a["mass"]
        co = ((masses[i] == 12.0) & (masses[j] == 15.999)) | ((masses[j] == 12.0) & (masses[i] == 15.999))
        allowed = co if int(g[37]) != 2 else onp.ones(len(i), dtype=bool)
        selected = onp.flatnonzero(allowed & (getval(bo) >= 1.0) & self.edges.half)
        if len(selected):
    ...
    def hydrogen_bonds(self, e, vectors, r, bo):
        ...
        sin4 = 0.25 * (1 - np.clip(cos, -1.0, 1.0)) ** 2
        e["hydrogen_bond"] = np.sum(
            hb1 * (1 - np.exp(-hb2 * bo[donor])) * np.exp(-hb3 * (r0 / distance + distance / r0 - 2)) * sin4
        )
    ...
    def angles(self, e, vectors, r, bo, pi, pp, total, d, db, nlp, vlpex, neighbors):
        ...
        sbo2 = np.where(
            sbo <= 0,
            0,
            np.where(sbo <= 1, positive_power(sbo, g[16]), np.where(sbo < 2, 2 - positive_power(2 - sbo, g[16]), 2)),
        )
        ...
        e["angle_conjugation"] = np.sum(
            coa1
            / (1 + np.exp(g[2] * (total[j] - a["valency_val"][j])))
    ...
    def torsions(self, e, vectors, r, bo, pi, db, neighbors):
        ...
        if onp.any(getval(sine1_sq) < 1e-20) or onp.any(getval(sine2_sq) < 1e-20):
            raise ValueError(
                "Collinear atoms in an active torsion: the dihedral derivative is undefined; perturb the geometry"
            )
        ...
        cv = 0.5 * (v1 * (1 + cw) + v2 * ex1 * (2 - 2 * cw * cw) + v3 * (1 + 4 * cw**3 - 3 * cw))
