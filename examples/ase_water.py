...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "validation/runs" /
                        ("ase-water-"+datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")))
    ...
    with optimizer_class(atoms, logfile=str(args.output / "optimize.log"),
                         trajectory=str(args.output / "relaxation.traj")) as optimizer:
    ...
    report = {"optimizer": f"ASE {args.optimizer}", "ase_version": version("ase"),
              "force_field_sha256": ff.checksum, "full_derivative": False,
              "force_convention": "fixed_charge", "energy_units": "eV", "force_units": "eV/Angstrom",
              "initial_energy": initial_energy, "energy": atoms.get_potential_energy(),
              "charges": atoms.get_charges().tolist(), "dipole_e_A": atoms.get_dipole_moment().tolist(),
              "max_atomic_force_norm": float(np.max(np.linalg.norm(forces, axis=1))),
              "fmax": 1e-5, "converged": bool(converged), "steps": steps}
    if args.verify:
    ...
    print(f"ASE {args.optimizer}: {'PASS' if report['passed'] else 'FAIL'}, {steps} steps, E={report['energy']:.10f} eV")
