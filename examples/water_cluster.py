...
def water_cases():
    ...
    monomer = np.array([[0.0, 0.0, 0.0], [0.97, 0.0, 0.0], [-0.243, 0.94, 0.0]])
...
def serialize(result):
    fields = ("energy", "forces", "charges", "components", "dipole", "total_bond_orders", "lone_pairs", "bond_counts")
...
def comparison(actual, reference, n):
    ...
    limits = {
        "energy_per_atom": 1e-5,
        "components_per_atom": 1e-5,
        "forces": 1e-4,
        "charges": 1e-6,
        "dipole": 1e-6,
    ...
    return {"maximum_absolute_errors": errors, "limits": limits, "passed": all(errors[k] <= limits[k] for k in errors)}
...
def main():
    ...
    for name, (symbols, positions) in water_cases().items():
        ...
        print(
            f"{name}: E={result.energy:.9f} kcal/mol"
