...
def small_cell_cases():
    ...
    cases = {f"water_{side:g}A": (water, symbols, x, np.diag([side]*3), [True]*3)
             for side in (9., 6., 4.)}
    ...
    cases["water_dimer_6A"] = (water, ds, dx, np.diag([6.]*3), [True]*3)
    tilted = np.array([[4.5, 0, 0], [1., 5., 0], [.6, .4, 5.5]])@rotation
    ...
    cases["zno_4A"] = ("ffield.reax.ZnOH", ["Zn", "O"], np.array([[0., 0, 0], [1.9, .1, .2]]),
                        np.diag([4.]*3), [True]*3)
    cases["zinc_chain"] = ("ffield.reax.ZnOH", ["Zn"], np.zeros((1, 3)), np.diag([2.5, 12., 12.]),
                           [True, False, False])
    return cases
...
def main():
    ...
    summary = {"full_derivative": False, "force_convention": "fixed_charge",
    ...
    for name, (filename, symbols, x, cell, pbc) in small_cell_cases().items():
        ...
        (work/"structure.json").write_text(json.dumps(dict(symbols=symbols, positions=x.tolist(),
             cell=cell.tolist(), pbc=pbc), indent=2)+"\n")
        start = perf_counter()
        ...
        row = {"atoms": len(x), "internal_atoms": int(len(x)*np.prod(result.cell_repetitions)),
               "cell_repetitions": result.cell_repetitions, "force_field": filename,
               "force_field_sha256": ff.checksum, "energy": result.energy,
               "hydrogen_bond_energy": result.components["hydrogen_bond"], "evaluation_seconds": elapsed}
        if args.verify:
        ...
        print(f"{name}: E={result.energy:.9f}, {len(x)} atoms / {row['internal_atoms']} internal, "
              f"{elapsed:.3f}s"+(f", {'PASS' if row['passed'] else 'FAIL'}" if args.verify else ""), flush=True)
    if args.verify:
