...
def water_box(counts=(4, 4, 4), cell=None, pbc=(True, True, True)):
    ...
    for index in product(*(range(n) for n in counts)):
        center = ((np.asarray(index)+.12)/counts) @ cell + rng.normal(0, .04, 3)
...
def periodic_cases():
    ...
    separated[3:] += [3.15, .2, .3]
    tilted = np.array([[12., 0, 0], [3., 12., 0], [1., 2., 12.]])
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "validation" / "runs" /
                        ("periodic-"+datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")))
    args = parser.parse_args()
    ff = ForceField.bundled("qeq_ff.water")
    calc = Calculator(ff)
    args.output.mkdir(parents=True, exist_ok=False)
    summary = {"force_field": ff.path.name, "sha256": ff.checksum,
               ...
               "environment": {"python": sys.version, "executable": sys.executable,
                               "platform": platform.platform(), "numpy": version("numpy"),
    ...
    for name, (symbols, positions, cell, pbc) in periodic_cases().items():
        ...
        row = {"atoms": len(symbols), "energy": result.energy, "evaluation_seconds": elapsed,
               "cell": cell.tolist(), "pbc": pbc, "total_charge": float(result.charges.sum())}
        if args.verify:
            ref = evaluate_lammps(ff, symbols, positions, cell=cell, pbc=pbc,
                                  directory=directory/"lammps", executable=args.executable)
        ...
        print(f"{name}: E={result.energy:.9f} kcal/mol, {elapsed:.3f}s" +
              (f", reference {'PASS' if row['passed'] else 'FAIL'}" if args.verify else ""), flush=True)
    if args.verify:
