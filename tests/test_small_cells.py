...
def test_small_cell_memory_limit(tmp_path):
    ...
    with pytest.raises(ValueError, match="max_expanded_atoms"):
        Calculator(ff, max_expanded_atoms=80).evaluate(symbols, x, cell=[4.]*3)
    with pytest.raises(ValueError, match="max_expanded_atoms"):
        evaluate_lammps(ff, symbols, x, cell=[4.]*3, directory=tmp_path/"reference", max_expanded_atoms=80)
...
def test_supported_small_cells(name, tmp_path):
    ...
    if name == "water_4A":
        assert result.components["hydrogen_bond"] == pytest.approx(-.3700520183, abs=1e-9)
    if name == "zinc_chain":
        assert result.bond_orders[0, 0] > 1.
        assert result.bond_counts.tolist() == [2]
        assert result.total_bond_orders[0] == result.bond_orders[0, 0]
        np.testing.assert_allclose(result.forces, 0., atol=1e-12)
...
def test_small_cell_symmetries(name):
    ...
    transformed = calc.evaluate(symbols, x@rotation+[12., -8., 3.], cell=cell@rotation, pbc=pbc)
...
def test_small_cell_relaxation(backend, tmp_path, monkeypatch):
    ...
    x = np.array([[0., 0, 0], [.97*np.cos(half), .97*np.sin(half), 0],
