...
def test_periodic_reference(calc, name, tmp_path):
    ...
    if name == "multiple_images":
        ...
        assert abs(actual.energy-isolated.energy) > .01
    if name == "bulk_water_192":
        assert actual.components["hydrogen_bond"] < -10
        assert abs(actual.components["torsion"]) > .01
...
def test_wrapping_translation_rotation_and_permutation(calc, name):
    ...
    transformed = calc.evaluate(symbols, x@rotation+[21., -5., 40.], cell=cell@rotation, pbc=pbc)
    ...
    np.testing.assert_allclose(original.forces.sum(axis=0), 0., atol=1e-9)
...
def test_supercell_extensivity(calc):
    ...
    for name in ("charges", "forces", "total_bond_orders", "lone_pairs", "bond_counts"):
        np.testing.assert_allclose(getattr(doubled, name), np.concatenate([getattr(original, name)]*2),
                                   atol=1e-9, rtol=0)
...
def test_periodic_input_semantics(calc):
    ...
    display = calc.evaluate(symbols, x, cell=[2., 2., 2.], pbc=False)
    assert display.energy == original.energy
    implicit = calc.evaluate(symbols, x, cell=[12., 12., 12.])
    explicit = calc.evaluate(symbols, x, cell=np.diag([12.]*3), pbc=True)
    assert explicit.energy == implicit.energy
    # Only periodic directions require cell heights larger than the cutoff.
    calc.evaluate(symbols, x, cell=[12., 12., 3.], pbc=[True, True, False])
    with pytest.raises(ValueError, match="Coincident"):
        calc.evaluate(["O", "H"], [[0., 0, 0], [12., 0, 0]], cell=[12.]*3)
...
@pytest.mark.reference
@pytest.mark.parametrize("ffield,symbols,x", [
    ("ffield.reax.cho", ["C"]*4, [[0, 0, 0], [1.5, .1, 0], [2.3, 1.2, .2], [3.7, .9, .8]]),
    ("ffield.reax.ZnOH", ["Zn", "O"], [[0., 0, 0], [1.9, .1, .2]]),
])
def test_other_force_fields(ffield, symbols, x, tmp_path):
    ff = ForceField.bundled(ffield)
    x = (np.asarray(x)+[11.5, 11.5, 11.5]) % 12
    result = Calculator(ff).evaluate(symbols, x, cell=[12.]*3)
    ref = evaluate_lammps(ff, symbols, x, cell=[12.]*3, directory=tmp_path/"reference")
    report = comparison(result, ref, len(x))
    assert report["passed"], report
    if ffield.endswith("cho"):
        assert abs(result.components["torsion"]) > .1
