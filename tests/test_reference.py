...
def test_reference(name, tmp_path):
    ...
    np.testing.assert_allclose(actual.lammps_forces, reference.forces, atol=1e-4, rtol=0)


@pytest.mark.parametrize("distance", [1.5, 2.5, 3., 3.5, 4., 4.9999, 5.0001, 9.999, 10., 10.001])
...
def test_reference_relaxed_geometry(name, tmp_path):
    ...
    np.testing.assert_allclose(relaxed.evaluation.lammps_forces, reference.forces, atol=1e-4, rtol=0)
